...
def simulated_annealing(coordinates, initial_temp=1000, cooling_rate=0.995, min_temp=1e-4, max_iterations=1000):
    ...
    while temp > min_temp and iteration < max_iterations:
        ...
        new_distance = calculate_distance(new_route, coordinates)
...
def main():
    ...
    print(f"Running Simulated Annealing for {runs} iterations...")

    for run in range(runs):
        print(f"Run {run + 1}/{runs}...")
        start_time = time.time()
        best_route, best_distance, convergence = simulated_annealing(coordinates)
        end_time = time.time()
